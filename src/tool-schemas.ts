...
function executionInputProperties() {
  return {
    command: { type: 'string', minLength: 1, description: 'Non-interactive shell command evaluated by the required selected shell. shell=\"sh\" uses /bin/sh -c and shell=\"bash\" uses /bin/bash -c. Use explicit quoting for pipelines, redirection, &&, and environment expansion. Do not use this tool for an interactive shell, an intentionally unbounded service, or file transfer.' },
    shell: { type: 'string', enum: ['sh', 'bash'], description: 'Required shell interpreter for command. Use sh for portable POSIX-style commands and bash whenever the command uses Bash-specific syntax such as [[ ]], arrays, ${var//old/new}, or set -o pipefail. Execution uses /bin/sh -c or /bin/bash -c; bash requires /bin/bash on the remote host. Login and interactive shells are not used.' },
...
function execToolSchema() {
  return {
    ...
    description: 'Run one bounded, non-interactive shell command synchronously and wait for its terminal result. Task-context rule: before the first execution in a new ChatGPT conversation/logical task, call begin_task once; then reuse its exact task_handle on every exec/start_exec in that same task. Agent selection guidance: prefer exec only when the command is expected to finish quickly (roughly within 5 seconds) AND the next reasoning or action depends on its result. Typical exec cases are pwd, ls, cat/sed/grep, git status/diff, kubectl get, and other small deterministic probes or edits. For builds, test suites, package installs, image builds, scans, sleeps/waits, commands with uncertain duration, or work that can run in parallel, prefer start_exec so useful agent work can continue while the job runs. Never emulate background execution inside exec with nohup, disown, or a trailing/background ampersand; submit the foreground command through start_exec so Job Manager status, cancellation, timeout, and remote process-group cleanup remain authoritative. Do not use exec merely to wait for an asynchronous job; use get_exec_status at the synchronization point. Internally exec submits a sync job through the same Job Manager and runner used by start_exec, waits for a terminal state, then returns the final summary. Every command must explicitly select shell=sh or shell=bash. Use bash for Bash-specific syntax; execution uses /bin/sh -c or /bin/bash -c. The server validates cwd against ALLOWED_CWDS, enforces runtime and output limits, filters environment variables, enforces dedicated sync/global concurrency limits, and terminates the managed remote process group on timeout or cancellation.',
...
function derivedDiagnosticsSchemaProperties() {
  return {
    timings: {
      type: 'object',
      description: 'Durations derived from existing lifecycle timestamps; unavailable phases are null and no additional timing state is stored.',
      properties: {
        queue_ms: { type: ['integer', 'null'], minimum: 0, description: 'Time from queued to starting.' },
        transport_startup_ms: { type: ['integer', 'null'], minimum: 0, description: 'Time from scheduler starting until the local transport process reaches runner running state. This does not prove SSH handshake completion or remote process startup.' },
        time_to_first_output_ms: { type: ['integer', 'null'], minimum: 0, description: 'Time from runner running state to first stdout/stderr output; this can include transport connection, remote shell/application startup, and application silence.' },
        runtime_ms: { type: ['integer', 'null'], minimum: 0, description: 'Execution running time until abort request, terminal completion, or current query time.' },
        termination_ms: { type: ['integer', 'null'], minimum: 0, description: 'Time from abort request to terminal finalization when an abort occurred.' },
        total_ms: { type: 'integer', minimum: 0, description: 'Total age from job registration to terminal completion or current query time.' }
      },
      required: ['queue_ms', 'transport_startup_ms', 'time_to_first_output_ms', 'runtime_ms', 'termination_ms', 'total_ms'],
      additionalProperties: false
    },
    diagnostics: {
      type: 'object',
      description: 'Small deterministic diagnostic classification derived only from observable lifecycle facts; it does not claim that a process is hung or stuck.',
      properties: {
        phase: { type: 'string', enum: ['queued', 'starting', 'running', 'terminating', 'finished'], description: 'Current coarse execution lifecycle phase.' },
        activity: { type: 'string', enum: ['active', 'quiet', 'long_quiet', 'unknown'], description: 'Output/activity classification. quiet and long_quiet describe observation, not a hang diagnosis.' },
        failure_phase: { type: ['string', 'null'], enum: ['transport', 'execution', 'termination', null], description: 'Coarse lifecycle phase containing the failure when terminal evidence is sufficient; this is not a root-cause claim. null otherwise.' },
        last_activity_age_ms: { type: ['integer', 'null'], minimum: 0, description: 'Age of the most recent observed lifecycle/output activity at query or completion time.' },
        last_output_age_ms: { type: ['integer', 'null'], minimum: 0, description: 'Age of the most recent observed stdout/stderr output at query or completion time.' }
      },
      required: ['phase', 'activity', 'failure_phase', 'last_activity_age_ms', 'last_output_age_ms'],
      additionalProperties: false
    }
  };
}
...
function statusOutputProperties() {
  return {
    ...derivedDiagnosticsSchemaProperties(),
