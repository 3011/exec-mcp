...
export function renderMetrics(runner: ExecRunner): string {
  ...
  lines.push(`exec_mcp_max_concurrent_execs ${runner.config.globalMaxConcurrentExecs}`);
  lines.push('# HELP exec_mcp_queued_execs Number of queued executions waiting for an admission slot');
  lines.push('# TYPE exec_mcp_queued_execs gauge');
  lines.push(`exec_mcp_queued_execs ${runner.queued}`);
  lines.push('# HELP exec_mcp_total_active_jobs Number of all non-terminal executions including queued jobs');
  lines.push('# TYPE exec_mcp_total_active_jobs gauge');
  lines.push(`exec_mcp_total_active_jobs ${runner.activeJobs}`);
  lines.push('# HELP exec_mcp_sync_running Current executions consuming sync admission slots');
  lines.push('# TYPE exec_mcp_sync_running gauge');
  lines.push(`exec_mcp_sync_running ${runner.listActive().sync_running}`);
  lines.push('# HELP exec_mcp_async_running Current executions consuming async admission slots');
  lines.push('# TYPE exec_mcp_async_running gauge');
  lines.push(`exec_mcp_async_running ${runner.listActive().async_running}`);
  lines.push('# HELP exec_mcp_sync_max_concurrent Configured sync execution slot limit');
  lines.push('# TYPE exec_mcp_sync_max_concurrent gauge');
  lines.push(`exec_mcp_sync_max_concurrent ${runner.config.syncMaxConcurrentExecs}`);
  lines.push('# HELP exec_mcp_async_max_concurrent Configured async execution slot limit');
  lines.push('# TYPE exec_mcp_async_max_concurrent gauge');
  lines.push(`exec_mcp_async_max_concurrent ${runner.config.asyncMaxConcurrentExecs}`);
  lines.push('# HELP exec_mcp_global_max_concurrent Configured global running execution slot limit');
  lines.push('# TYPE exec_mcp_global_max_concurrent gauge');
  lines.push(`exec_mcp_global_max_concurrent ${runner.config.globalMaxConcurrentExecs}`);
  lines.push('# HELP exec_mcp_max_queued_execs Configured maximum queued executions');
  lines.push('# TYPE exec_mcp_max_queued_execs gauge');
  lines.push(`exec_mcp_max_queued_execs ${runner.config.maxQueuedExecs}`);
